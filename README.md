...
Punisher is a web HTTP proxy written in PHP. It is designed to bypass proxy restrictions through a web interface very similar to the popular [CGIProxy](http://www.jmarshall.com/tools/cgiproxy/). The only thing that Punisher needs is a web server with PHP installed (see Requirements below). Be aware though, that the sever has to be able to access those resources to deliver them to you.
 ...
 * Create an issue: https://github.com/Punisher/Punisher/issues/new
...
```
cd /var/www/html/
git clone https://github.com/donPabloNow/punisher.git
```
...
Punisher also doesn't support FTP. This may or may not be introduced 
