...
if(!isset($pos) || $pos == ""){ // empty means old method
  $_user_agent = isset($_SERVER['HTTP_X_IORG_FBS']) ? 'SamsungI8910/SymbianOS/6.1 Punisher/'.$_version : $_SERVER['HTTP_USER_AGENT'];
...
else
{
    ...
    {
        $_url_form      = '<div style="width: 100%; margin: 0; text-align: center; border-bottom: 1px solid #725554; color: #222; background-color: #000; font-size: 12px; font-family: Bitstream Vera Sans,arial,sans-serif; position: fixed; padding: 4px; bottom: 0px; font-weight: normal; text-align: left;">'
                        . '<form method="post" action="' . $_script_url . '">'
                        . '<input style="border-radius: 2px;width: 100%; border: none; margin: 0px; margin-bottom: 10px; background: #111; color: #555; padding: 10px;" id="____' . $_config['url_var_name'] . '" type="text" size="80" name="' . $_config['url_var_name'] . '" value="' . $_url . '" />';
        ...
        $_url_form .= '[go: <a style="color:#fff"  href="' . $_script_url . '?' . $_config['url_var_name'] . '=' . encode_url($_url_parts['prev_dir']) .' ">up one dir</a>, <a style="color:#fff"  href="' . $_script_base . '">main page</a>]<input style="position: absolute; right: 0px; bottom: 0px; margin: 3px; padding: 3px 40px; background: #590000; border: none; border-radius: 2px; cursor: pointer;" type="submit" name="go" value="Go" /></form></div>';
