...
# ESSENTIAL if you're distributing a asset or plugin.
define('COMPATABILITY_MODE', false);

# Set up paths/urls
define('PUNISH_ROOT', str_replace('\\', '/', dirname(dirname(__FILE__))));
define('PUNISH_URL',
    'http'
    . (HTTPS ? 's' : '')
    . '://'
    . $_SERVER['HTTP_HOST']
    . preg_replace('#/(?:(?:inc/)?[^/]*|' . preg_quote(SCRIPT_NAME) . '.*)$#', '', $_SERVER['PHP_SELF'])
);
define('PUNISH_BROWSE', PUNISH_URL . '/' . SCRIPT_NAME);
...
function proxyURL($url, $givenFlag = false)
{
    ...
    # Validate the input
    if (empty($url) || $url[0] == '#' || $url == 'about:' || stripos($url, 'data:') === 0 || stripos($url, 'file:') === 0 || stripos($url, 'res:') === 0 || stripos($url, 'C:') === 0 || strpos($url, PUNISH_BROWSE) === 0) {
        return '';
    }
    ...
    return PUNISH_BROWSE . '?u=' . $url . '&b=' . $bitfield . ($addFlag ? '&f=' . $addFlag : '') . $anchor;
}
...
function deproxyURL($url, $verifyUnique = false)
{
    ...
    # Remove our prefix
    $url = str_replace(PUNISH_BROWSE, '', $url);
...
function loadTemplate($file, $vars = array())
{
    ...
    if (empty($template)) {
        ...
        return '<b>ERROR:</b> template failed to load. Please ensure you have correctly installed any custom assets and check you have not removed any files from the default asset.';

    }

    # Apply asset replacements to template
    $template = replaceAssetTags($template);
...
# Make asset tag replacements
function replaceAssetTags($template)
{

    global $assetReplace;

    if (!empty($assetReplace)) {

        foreach ($assetReplace as $tag => $value) {
            ...
            if (COMPATABILITY_MODE) {
                $template = str_replace('<!--[punisher:' . $tag . ']-->', $value, $template);
            }
...
function replaceContent($content)
{
    ...
    # Return with asset tags replaced
    return replaceAssetTags(preg_replace('#<!-- CONTENT START -->.*<!-- CONTENT END -->#s', $content, $output));
}
...
function error($type, $allowReload = false)
{
    ...
    # Finally add it to the $assetReplace array to get it in there
    $assetReplace['error'] = '<div id="error">' . $errorText . '</div>';

    # And a link to try again?
    $return = currentURL();
    if (strlen($return) > 0) {
        $assetReplace['error'] .= '<p style="text-align:right">[<a href="' . htmlentities($return) . '">Reload ' . htmlentities(deproxyURL($return)) . '</a>]</p>';
    }
...
function currentURL()
{
    ...
    # Return full URL
    return PUNISH_BROWSE . $separator . (isset($_SERVER[$method]) ? $_SERVER[$method] : '');
}
